...
|---|---|
| 窗口 | 本地 **昨天** 自然日 |
| 自动 | 每天 **08:00**（或下次冷启动补跑）生成昨天日报并落盘 |
| **呈现** | **仅在历史弹窗新增「日报」Tab**；列表/详情在该 Tab 内打开已生成的 HTML（或降级页） |
| **不做** | 邮件代发、mailto、系统分享入口、独立 `/day` 全屏阅读器、桌面 WeeklyReader 复用为必选项、主动推送通知（均可后置，**非 v1**） |
| `/mood` | 并存（近 60 轮即时总结）；与历史「日报」Tab 无关 |
| 周报 / reflect | 独立；不混 |

### 1.0 呈现方式（已拍板）

- 入口：**历史弹窗**（现有 HistoryOverlay：历史 / 收藏）→ 增加第三 Tab：**日报**。  
- 交互：Tab 内按日期列出已生成日报（最近 N 天）；点一项 → **同弹窗内**展开/切到详情读 HTML（或内嵌简易 WebView/iframe）；关闭仍走历史弹窗关闭。  
- 无数据：空态文案（「昨天还没有足够记录」/「日报尚未生成」），不跳转外链、不引导邮箱。  
- **零侵入样式**：只在 HistoryOverlay 增 Tab 与列表/详情区域；不改首页、锁定钮、播放条布局。
...
- 在 HistoryOverlay 增加「日报」Tab + 列表/详情（**仅此为用户入口**）  
...
- 改首页布局 / 锁定钮样式（埋点除外）；历史弹窗只加 Tab，不重做整窗视觉  
- 邮件、分享、推送、独立日报路由作为 v1 范围  
- 把日报结论写回推荐或自动开锁  
...
历史弹窗
  [播放历史] [收藏] [日报]   ← 新增
                    │
                    ├─ 列表：2026-08-10 · 有结论 3 条 …
                    └─ 详情：结论 + 日信 + 详细数据（同弹窗内）
```

详情内容结构仍为：结论摘要 → 日信（可无）→ 详细数据（含锁定专表）。  
视觉：跟历史弹窗现有语言一致（Tab 样式复用），详情 HTML 可用独立 renderer，**不新开全屏页、不发邮件**。
...
| **P2** | 历史弹窗「日报」Tab + 列表/详情读盘；后台生成 | Tab 能打开昨天日报；无数据有空态 |
| **P3** | 天气快照等增强 | 不挡 P2 |
...
- `/mood` 并存；结论以规则为准  
- **用户入口只有历史弹窗「日报」Tab**；无邮件/无分享/无独立阅读器  
- iOS + core 先接锁定与播放埋点（锁定 UI 主要在 mobile）  
- 桌面无锁定钮则 `track_lock_*` 为空，Quality 标明；桌面若有历史面板可后跟同 Tab  
...
| 项 | v1 | v2 |
|---|---|---|
| 呈现 | `/day`、WeeklyReader、分享等 | **历史弹窗第三 Tab「日报」；其余呈现不做** |
