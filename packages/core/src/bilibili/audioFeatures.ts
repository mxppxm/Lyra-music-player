 ...
 *
 * On iOS, only bundled cache reads are supported — no on-device FFT.
 */
...
export async function readFeatureCache(): Promise<FeatureCache> {
  ...
  const raw = await getLyraPlatform().readFeatureCache();
  cache = (raw ?? {}) as FeatureCache;
  return cache;
}
...
export async function writeFeatureCache(): Promise<void> {
  ...
  try {
    await getLyraPlatform().writeFeatureCache(cache as unknown as Record<string, unknown>);
    cacheDirty = false;
  } catch (e) {
 ...
 *
 * On iOS: cache-only — returns null when uncached (no Rust FFT available).
 */
export async function getOrAnalyzeFeatures(
  ...
  // 2. Download + analyze via Rust (desktop only)
  try {
    const platform = getLyraPlatform() as unknown as {
      analyzeAudioUrl?: (url: string) => Promise<AudioFeatures>;
    };
    if (!platform.analyzeAudioUrl) return null;

    const features: AudioFeatures = await platform.analyzeAudioUrl(audioUrl);
